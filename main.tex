...
% Fallback notation for digital typesetting and plain-text corpora
% Use these when subscript typography is unavailable
% Examples: noun_Cross, noun_Eng, noun_L
% In LaTeX, these commands expand to the subscripted versions
\newcommand{\Cross}{\textsubscript{\Cross}}
\newcommand{\Eng}{\textsubscript{Eng}}
\newcommand{\Jpn}{\textsubscript{Jpn}}
\newcommand{\Spn}{\textsubscript{Spn}}
\newcommand{\Tha}{\textsubscript{Tha}}
\newcommand{\Rus}{\textsubscript{Rus}}
\newcommand{\Lat}{\textsubscript{Lat}}
\newcommand{\Hbr}{\textsubscript{Hbr}}
\newcommand{\Lgr}{\textsubscript{$L$}}  % Generic language variable
...
\noindent\textbf{Note on notation:} The subscript convention is elegant for print but problematic for digital typesetting and plain-text corpora. For databases, code, and plain-text applications, use the fallback notation: \texttt{noun\_Cross}, \texttt{noun\_Eng}, \texttt{noun\_Jpn}, \texttt{noun\_L}. In LaTeX, these commands (\verb|\Cross|, \verb|\Eng|, etc.) expand to the subscripted versions defined above.
...
\begin{description}
  ...
  \item[Analyst weight] A confidence rating based on diagnostic strength: $w_L^{\text{analyst}}(c,\phi)$ is assigned via a four-point ordinal scale derived from the hygiene protocol (Section~\ref{subsec:hygiene-procedure}):
  \begin{itemize}
  ...
  \end{itemize}
  Analysts document which diagnostics support the assignment; inter-coder agreement (Cohen's $\kappa > 0.7$) validates the scoring.
\end{description}

\noindent Even better, treat $w_L$ as a \emph{parameter to be estimated} by the measurement model (Section~\ref{sec:measurement}) rather than pre-specified. In this approach, each diagnostic $d_i$ provides evidence for the latent comparandum $\eta_c$, and the model estimates the mapping strength via:
\[
  \text{logit}(p(d_i = 1 \mid \eta_c, L)) = \alpha_i + \beta_i \cdot w_L(c, \phi) \cdot \eta_c
\]
where $w_L(c,\phi)$ is estimated as part of the model, constrained by partial pooling across languages and families. This avoids circularity: the weights are \emph{inferred} from diagnostic patterns rather than stipulated by analysts.

In practice, implementations should use \textbf{triangulation}: start with analyst weights for pilot coding, validate against corpus-based observational weights where available, then refine via measurement model estimation. The reliability score $r_L(c,\phi)$ captures residual uncertainty:
\[
...
Rule~B (\textbf{No meaning-as-form identity}) is not merely a coding convenience~-- it is theoretically essential. Without it, typology commits a category error: treating a semantic target as if it were a morphological exponent. This conflation has three fatal consequences:
...
\subsection{Coding protocol (excerpt)}\label{subsec:hygiene-protocol}

For each Level~I comparandum $c$ and language $L$, follow this five-step protocol:

\begin{enumerate}[label=\arabic*., leftmargin=*]
  \item \textbf{Establish diagnostic traction.} Verify behavioural tests yield consistent judgments in $L$. If diagnostics fail, refine tests or recognize $c$ may not be relevant for $L$.
  
  \item \textbf{Apply diagnostics to contexts.} Test NPs/clauses using operational criteria below. Score evidence: 0 = fails; 1 = weak; 2 = moderate; 3 = strong. Conduct this step \emph{without consulting} morphosyntactic form.
  
  \item \textbf{Identify candidate realizations.} Only after step~2, survey which forms $\phi$ correlate with high-scoring contexts.
  
  \item \textbf{Score mappings.} Assign $w_L(c, \phi)$ based on sensitivity (recall), precision, and contextual strength (Section~\ref{subsec:weight-procedures}).
  
  \item \textbf{Record provenance and reliability.} Document inter-coder agreement, experimental validation, or corpus evidence.
\end{enumerate}
...
\begin{description}
  ...
  \]
  where $\tau_1 = 0.6$ (no canonical exponent) and $\tau_2 = 2.0$ (too many weak correlates). This signals that $c$ is not a unified target but a grab-bag of unrelated exponents.
  ...
  \]
  where $n_{\text{min}} = 10$ languages with moderate or better reliability. Without adequate coverage, naturalization claims cannot be evaluated.
\end{description}
...
As a theoretician, I can formalize these mechanisms probabilistically, making them operational for empirical testing:
...
\begin{description}
  ...
  \]
  where $\delta = 0.15$ and $P \in \{\text{reference tracking}, \text{individuation}, \text{possession}, \text{quantification}\}$. This predicts that languages under specific functional pressures will reliably develop forms realizing the target comparandum.
  ...
  \]
  where $\epsilon = 0.4$ (weak correlate threshold) and $\gamma = 0.10$. This captures paradigmatic pressure: if a form realizes one comparandum strongly, it's more likely to realize related comparanda, creating cohesive clusters.
  ...
  \]
  where $\eta = 0.60$ over a 500-year window. This quantifies the claim that when a language loses one exponent of a comparandum, it reliably develops a replacement via grammaticalization pathways (demonstrative $\to$ article, measure term $\to$ classifier, etc.).
\end{description}
...
As a theoretician, I can \emph{derive} the measurement structure directly from the three-level ontology rather than asserting it post-hoc. Each Level-I target is a latent construct $\eta_c$ that causes observable diagnostics via homeostatic mechanisms. The model is:

\begin{align}
  \eta_c &\sim \text{Normal}(0, \sigma_\eta) && \text{(latent comparandum)} \\
  d_i &\sim \text{Bernoulli}(\pi_i) && \text{(diagnostic response)} \\
...
This derivation shows the measurement model isn't just a statistical add-on but a \emph{formal consequence} of the theory: Level-I targets are latent constructs, Level-II diagnostics are noisy indicators, and Level-III forms are the observable exponents mediated by homeostatic mechanisms.
...
\paragraph{Coding burden} Populating $M_L$ for even 50 languages with required reliability is a multi-year, multi-researcher project. The "LLMs as research assistants" footnote is realistic, but requires explicit prompt engineering, validation pipelines, and error propagation protocols.

\paragraph{Diagnostic design} Definiteness diagnostics (anaphoric uptake, bridging) require controlled discourse contexts that don't exist for most languages. Adaptation to elicited narratives or corpus-mining heuristics is necessary, with validation against expert judgments.

\paragraph{Weight calibration} Without large parallel corpora, observational weights (Section~\ref{subsec:weight-procedures}) are impossible. Analyst weights based on diagnostic strength are necessary, reintroducing subjectivity. The reliability score $r_L$ must capture this uncertainty formally (e.g., as precision parameter in hierarchical model).

\paragraph{Genealogical stratification} Predictions require phylogenetically controlled samples. Most typological databases (WALS, Grambank) don't stratify this way; custom sampling is needed with explicit family/area controls.

\paragraph{Literacy as moderator} Predictions about mechanism competition (Section~\ref{subsec:mechanism-predictions}) require stratifying by literacy availability. This demands historical sociolinguistic data often unavailable for minority languages.
...
These predictions are \emph{uniquely mechanistic}: they test not just correlations but the causal processes (regeneration pathways, competition between cognitive vs. material pressures, co-adaptive compensation) that maintain homeostasis.
...
Some will read "naturalized" as sneaking back in innate categories or semantic essences. This misinterprets the claim. Naturalization is a \emph{hypothesis about process convergence}, not a claim about innate primitives.
