...
\author{Brett Reynolds \orcidlink{0000-0003-0073-7195}\thanks{I used ChatGPT 5, Claude Sonnet 4.5, Gemini Pro 2.5, and Kimi 2 extensively in drafting and revising the paper. I reviewed, edited, and approved all the material and take full responsibility for the final text and conclusions. I'm grateful to Martin Haspelmath for comments on an earlier draft. His identification of a notational slip in the worked illustrations~-- where I inadvertently treated the absence of language-internal categories as if it were a cross-linguistic generalization~-- led to important clarifications throughout Section~\ref{sec:illustrations}. All remaining errors are my own.
...
\begin{abstract}
Typological universals keep dissolving because we conflate what we compare with how languages realize it. We count articles and call it definiteness. We track nominative case and call it subjecthood. When English nouns or Hebrew subjects are projected directly onto purportedly universal categories, measurement error accumulates and generalizations fragment. Separating comparative concepts from language-specific realizations solves the conflation problem—but leaves a deeper question unanswered: which comparative concepts, if any, are more than analyst conveniences?

Some comparative concepts earn \emph{naturalized} status as homeostatic property cluster kinds—stable not because they are universal, but because convergent mechanisms maintain them across unrelated languages. Just as camera eyes evolved independently in vertebrates and cephalopods through convergent solutions to light detection, noun-like categories recur through convergent discourse and morphosyntactic pressures. This turns comparative concepts from descriptive tools into testable hypotheses about linguistic evolution with falsifiable predictions about regeneration, erosion, and trade-offs.

Naturalization requires disciplined separation of comparanda from realizations via explicit mapping functions, independent diagnosis of semantic targets through a syntax--semantics firewall, and measurement models with declared empirical thresholds for cross-linguistic clustering, mechanistic explanation, and predictive purchase. The framework is formalized in an auditable specification (\texttt{src/typology.py}) and issues risk-bearing predictions about diachronic pathways, functional trade-offs, and latent-variable diagnostics. By demanding mechanisms as well as mappings, the account preserves typology's empirical ambition while avoiding universalist overreach.
\end{abstract}

\section{Comparative concepts and the naturalization question}\label{sec:failures}
Comparative concepts solved typology's conflation problem. By treating cross-linguistic categories as analyst-defined tools for comparison rather than universal entities, Haspelmath's programme \parencite{Haspelmath2010} eliminates the confusion of projecting language-specific structures onto purportedly universal categories. Comparative concepts are descriptive conveniences—they allow us to ask whether English's noun and Japanese's noun realize similar patterns without presupposing universals. This is methodological progress.

But the solution leaves a question unanswered: are \emph{all} comparative concepts merely conveniences, or do some stabilize across unrelated languages because convergent mechanisms maintain them? If some concepts recur not through shared ancestry or borrowing but through independent evolutionary pressures—cognitive constraints, discourse requirements, diachronic pathways—then those concepts deserve promotion from instrumental tools to naturalized kinds with testable mechanistic explanations.

Without criteria for distinguishing naturalized concepts from inherited templates, typology remains vulnerable to four systematic errors. These obstacles manifest when we lack principled ways to determine whether cross-linguistic patterns reflect convergent stability or descriptive artifacts.

\subsection{Four obstacles manifested}\label{subsec:failure-modes}

\emph{Dual-use terminology} is typology's original sin. When \term{determiner} names both a function and a category, we stop distinguishing them. Confusion becomes invisible. A textbook example is \term{determiner}: it often names an NP-internal syntactic function and, at the same time, the lexical category realized in English by words like \textit{the}, \textit{this}, \textit{many}.  \textcite{Reynolds2025Definiteness} has recently pointed out that \textit{definite} is used as both a semantic term and for the exponent that prototypically realize definite semantics. \textit{Adverb}(\textit{ial}) commonly confounds adjuncts in general with a lexical category that commonly functions as adjuncts. \textit{Topic} is used multiply for categories (e.g., \textit{topic marker}), a discourse/pragmatic role, and prosodic/intonational patterns. Although many linguists and laypeople are aware of this polysemy and try to guard against it, confusion abounds. Maintaining strict terminological discipline is best practice, even if that means creating vulgar neologisms.
...
These obstacles persist because comparative concepts as currently practiced provide no mechanism for distinguishing convergent stability from template import. Terminological discipline prevents conflation, but it doesn't tell us which concepts deserve naturalization. \textsc{definiteness}\textsubscript{\Cross} and \textsc{specificity}\textsubscript{\Cross} recur across articleless languages and DOM-less languages—suggesting stable targets maintained by discourse mechanisms. But without explicit criteria for cross-linguistic clustering, mechanistic explanation, and predictive purchase, we cannot distinguish such patterns from descriptive conveniences. The framework developed below provides those criteria, making naturalization empirically defeasible rather than stipulative.
...
Populating $M_L$ for even 50 languages with required reliability is a multi-year, multi-researcher project. The \enquote{LLMs as research assistants} footnote is realistic, but requires explicit prompt engineering, validation pipelines, and error propagation protocols.
